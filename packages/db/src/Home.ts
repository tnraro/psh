...
export const getHomeById = async (
    pool: Pool,
    id: string
): Promise<IDBHome | null> => {
    const [
        rows
    ] = await pool.execute("SELECT * FROM `customer`.`Home` WHERE `tnid`=?", [
        id
    ]);
    const home = (<IDBHome[]>rows)[0];
    return home;
};
export const getHomeByUserId = async (
    pool: Pool,
    id: string
): Promise<IDBHome | null> => {
    const QUERY =
        "SELECT * FROM `customer`.`Home` WHERE `tnid`=(SELECT `homeId` FROM `customer`.`User` WHERE `tnid`=?);";
...
export const createHome = async (pool: Pool, home: IDBHome, userId: string) => {
    const INSERT_QUERY =
        "INSERT INTO `customer`.`Home` (`tnid`, `name`) VALUES (?,?);";
    const UPDATE_QUERY =
        "UPDATE `customer`.`User` SET `homeId`=? WHERE (`tnid`=?);";
