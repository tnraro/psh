...
import { Alert, AlertIcon, Avatar, Box, Button, Center, Heading, Input, Spacer, Stack, Stat, StatHelpText, StatLabel, toast, useToast, Wrap, WrapItem } from "@chakra-ui/react";
import { MyHomeQuery, useJoinHomeMutation, useMeQuery, useMyHomeQuery, useNewHomeMutation } from "@psh/schema/dist/generated/operations";
...
const NoHome = (props: any) => {
    ...
    return <Stack>
        ...
        <Stack direction={{ base: "column", md: "row" }} spacing={{ base: 5, md: 2 }}>
            <Stack border="1px" borderColor="blue.100" borderRadius={5} p={2} color="blue.400">
            ...
            <Stack border="1px" borderColor="cyan.100" borderRadius={5} p={2} color="cyan.400">
        ...
        <Box>
            ...
            <Wrap>
            {
                me?.home?.devices?.map(device => {
                    return <WrapItem border="1px" borderRadius=".8em" borderColor="gray.200" py={2} px={4}>
                        <Stat>
                            <StatLabel>{device?.alias}</StatLabel>
                            <StatHelpText>{device?.type}</StatHelpText>
                            {!device?.private || device?.owner?.id === me.id && <FiHeart />}
                        </Stat>
                    </WrapItem>;
                })
            }
